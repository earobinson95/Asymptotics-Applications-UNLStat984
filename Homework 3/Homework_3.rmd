...
Consider $I(X_i \le t)\sim Bern(t)$ for $0<t<1.$ Then, $$G_n(t) = \sum_{i=1}^nI\{X_i\le t\}\sim Bin(n, t).$

(b) Suppose $c>0.$ Find the distribution of a random variable $X$ such that $G_n(c/n)\overset{d}\rightarrow X$. Justify your answer.

Consider $Y_n\sim Bin(n, p_n)$ where $p_n \rightarrow c.$ Then for $y = 0, 1, 2, ...$,
\begin{align*}
...
& = \frac{c^y e^{-c}}{y!}.
\end{align*}
Therefore, $Y_n \overset{d}\rightarrow Poisson(c).$

Let $Y_n = G_n(c/n)\sim Bin(n, c/n).$ Then $n\cdot \frac{c}{n} \rightarrow c.$ Thus, using the result from above, $Y_n\overset{d}\rightarrow Poisson(c).$
...
We now have $I(X_i \le t)\sim Bern(1-e^{-t})$ for $0<t<1.$ Then, $G_n(t) = \sum_{i=1}^nI\{X_i\le t\}\sim Bin(n, 1-e^{-t}).$ Thus, let $Y_n = G_n(c/n)\sim Bin(n, 1-e^{-c/n}).$ Then using Taylors series expansion $$n(1-e^{-c/n})=n\left(1 - \left(1 - \frac{c}{n} + \frac{c^2}{2n^2}\right)\right) = c + o\left(\frac{1}{n}\right).$$ Thus, using the result from above, $Y_n\overset{d}\rightarrow Poisson(c).$
