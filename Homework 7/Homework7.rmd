...
Since $X\sim\text{Bern}(1/2)$ and $Y\sim\text{Bern}(1/2)$, we know $\sigma_x^2 = \sigma_y^2 = 1/4$. Since $P(XY=1)=\theta,$ we know $\sigma_xy = E[XY]-E[X]E[Y]=\theta-1/4$ and $\rho = 4\theta - 1.$ Then
...
We know, $\sqrt{n}(r_n-\rho) \overset{d}\rightarrow N(0, A\Sigma^{*} A^T)$ where
...
$$A\Sigma^{*}A^T = 128\theta^3-144\theta^2+40\theta.$$
Thus, since $\theta = \frac{\rho+1}{r},$
